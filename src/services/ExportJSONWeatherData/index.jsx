...
/**
 * ExportWeatherData JSON
 * - Read ?lat=&lng=&date=YYYY-MM-DD&hour=H (0-23)
 *
 * Note: Returns null (no HTML). The page must be used as the export endpoint.
 */
export default function ExportJSONWeatherData() {
    const downloaded = useRef(false); // avoid double download

    useEffect(() => {
        // prevents dual execution in Strict Mode (dual mounting in dev)
        ...
        (async () => {
            ...
            // fetch place name via Nominatim
            ...
            // fetch historical specific hour
            const fetchSpecificDateData = async (latv, lngv, dateStr, hourStr) => {
                try {
                    ...
                    const idx = data.hourly.time.findIndex(t => t.startsWith(dateStr) && new Date(t).getUTCHours() === parseInt(hourStr, 10));
            ...
            // climatology probabilistic (last 10 years)
                ...
                try {
                    ...
                    for (let ano of anos) {
                        ...
                        try {
                            ...
                            const idx = data.hourly.time.findIndex(t => t.startsWith(startDateStr) && new Date(t).getUTCHours() === parseInt(hourStr, 10));
                            if (idx === -1) continue;
                            ...
                            console.warn("Year fetch failed:", ano, err);
                            continue;
                        }
                    }
                    ...
                    const stats = {
                        t2m: { mean: average(samples.t2m), std: stdDev(samples.t2m), q1: percentile(samples.t2m, 0.25), q3: percentile(samples.t2m, 0.75), min: Math.min(...samples.t2m), max: Math.max(...samples.t2m), n: samples.t2m.length },
                        rh2m: { mean: average(samples.rh2m), std: stdDev(samples.rh2m), q1: percentile(samples.rh2m, 0.25), q3: percentile(samples.rh2m, 0.75), min: Math.min(...samples.rh2m), max: Math.max(...samples.rh2m), n: samples.rh2m.length },
            ...
            try {
                ...
                const isFuture = selected.getTime() > todayUTC.getTime();

                const result = isFuture
                    ? await fetchClimatologyProbabilities(lat, lng, date, hour)
                    : await fetchSpecificDateData(lat, lng, date, hour);
